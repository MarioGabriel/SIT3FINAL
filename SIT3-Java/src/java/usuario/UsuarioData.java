...
public class UsuarioData {
    ...
    public boolean darPrivilegio(DbTransaction tr, String username) throws Exception
    {
        UsuarioDO user = new UsuarioDO();
        user = selecionarPorUsername(tr,username);
        String privilegio = user.getPrivilegio();
        ...
        String query = "UPDATE usuario SET `privilegio`='"+privilegio+"' WHERE `username`='"+username+"'";
        DbCollection results = new DbCollection();
        tr.execute(query);
        if (user.getPrivilegio().equals(privilegio)){
            return true;
        }
        else{
            return false;
